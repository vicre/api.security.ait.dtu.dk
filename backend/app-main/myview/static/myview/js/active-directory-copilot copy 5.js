...
class App {
    constructor(uiBinder = UIBinder.getInstance()) {
        if (!App.instance) {
    ...
    async handleUserInput() {
        ...
        try {
            const res = await fetch('/active-directory/v1.0/query-assistant', {
                method: 'POST',
                headers: {
    ...
    static getInstance(uiBinder = UIBinder.getInstance()) {
        if (!App.instance) {
            App.instance = new App(uiBinder);
        }
        return App.instance;
    }
}
...
function getAuthToken() {
    // Replace this with your method of getting the auth token
    // For example, from a cookie, local storage, or a global variable
