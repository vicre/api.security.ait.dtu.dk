...
def run():
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "Giv mig en liste med alle de brugere som ikke har skiftet password siden 2020 under. Hvis kun brugere der ikke er disabled. Tilføj fæltet ou path"
        }
    ]

# "content": "Give me all PCs where last login is more five months ago, but less than 6 months ago, and is not disabled. Under institute SUS"
    ...
    # Initialize variables to store NT times
    nt_time_5_months_ago = None
    nt_time_6_months_ago = None
    ...
    while assistant_message.get("function_call"):
        ...
        if function_name == "get_nt_time_for_days_ago":
            ...
            nt_time_result = get_nt_time_for_days_ago(days_ago)

            # Store the NT time based on days_ago
            if days_ago == 150:  # Approximately 5 months
                nt_time_5_months_ago = nt_time_result
            elif days_ago == 180:  # Approximately 6 months
                nt_time_6_months_ago = nt_time_result
            ...
            # Replace placeholders with actual NT times
            search_filter = arguments["search_filter"]
            search_filter = search_filter.replace("{NT_TIME_5_MONTHS_AGO}", str(nt_time_5_months_ago))
            search_filter = search_filter.replace("{NT_TIME_6_MONTHS_AGO}", str(nt_time_6_months_ago))
