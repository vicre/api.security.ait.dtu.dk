...
if [ -d "$backend_dir/app-main" ] && [ -f "$venv_activate" ]; then
  cd "$backend_dir/app-main" && source "$venv_activate"
else
...
pip install -r "$backend_dir/app-main/requirements.txt" >/dev/null 2>&1 || true
